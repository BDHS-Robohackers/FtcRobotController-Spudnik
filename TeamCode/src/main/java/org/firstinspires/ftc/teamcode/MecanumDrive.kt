...
class MecanumDrive(hardwareMap: HardwareMap, var pose: Pose2d) {
    class Params {
        ...
        var logoFacingDirection: LogoFacingDirection = LogoFacingDirection.BACKWARD
        var usbFacingDirection: UsbFacingDirection = UsbFacingDirection.RIGHT

        // drive model parameters
        var inPerTick: Double = 0.02459268368
        var lateralInPerTick: Double = 0.02466091245
        var trackWidthTicks: Double = 1076.2880031505608

        // feedforward parameters (in tick units)
        var kS: Double = 1.5215977902641242
        var kV: Double = 0.004074151708786815
        var kA: Double = 0.0000007
        ...
        var lateralGain: Double = 5.0
        var headingGain: Double = 5.0 // shared with turn
    ...
    init {
        ...
        leftFront = hardwareMap.get(DcMotorEx::class.java, "fl_drv")
        leftBack = hardwareMap.get(DcMotorEx::class.java, "bl_drv")
        rightBack = hardwareMap.get(DcMotorEx::class.java, "br_drv")
        rightFront = hardwareMap.get(DcMotorEx::class.java, "fr_drv")
    ...
    inner class FollowTrajectoryAction(val timeTrajectory: TimeTrajectory) : Action {
        ...
        init {
            val disps = range(
                ...
                max(
                    2.0,
                    (ceil(timeTrajectory.path.length() / 2) as Int).toDouble()
                ).toInt()
            )
