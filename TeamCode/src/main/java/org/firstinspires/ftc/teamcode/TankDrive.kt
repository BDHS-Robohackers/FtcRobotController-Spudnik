...
class TankDrive(hardwareMap: HardwareMap, var pose: Pose2d) {
    class Params {
        ...
        var logoFacingDirection: LogoFacingDirection = LogoFacingDirection.UP
    ...
    inner class DriveLocalizer : Localizer {
        var leftEncs: List<Encoder>? = null
        var rightEncs: List<Encoder>? = null
        ...
        init {
            run {
                ...
                for (m in leftMotors) {
                    val e: Encoder = OverflowEncoder(RawEncoder(m))
                    leftEncs.add(e)
                }
        ...
        override fun update(): Twist2dDual<Time> {
            ...
            val twist: WheelIncrements<Time> = WheelIncrements<Time>(
                DualNum<Time>(
                    doubleArrayOf(
    ...
    fun setDrivePowers(powers: PoseVelocity2d) {
        val wheelVels: WheelVelocities<Time> = TankKinematics(2.0).inverse(
